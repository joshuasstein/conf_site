...
export default function AdminEditAbstractPage() {
  ...
  const handleSubmit = async (data: SubmissionCreate) => {
    ...
    try {
      ...
      router.push(`/admin/abstracts/${id}`);
    } catch (err: unknown) {
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">{submission.title}</p>
      </div>
      <AbstractForm
        defaultValues={{
